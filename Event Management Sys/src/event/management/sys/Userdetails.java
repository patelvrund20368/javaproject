...
public class Userdetails extends JFrame implements ActionListener {
    ...
    Userdetails() {
        ...
        JLabel heading = new JLabel("Search by User email");
        heading.setBounds(20, 20, 150, 20);
        add(heading);
        
        Uid = new Choice();
        Uid.setBounds(180, 20, 150, 20);
        add(Uid);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == search) {
            String query = "select * from user where email = '"+Uid.getSelectedItem()+"'";
            try {
                Conn c = new Conn();
